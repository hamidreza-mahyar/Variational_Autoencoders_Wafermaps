...
def generator_model(cat_dim, cont_dim, noise_dim):
    ...
    H.add(Dense(1024, kernel_initializer='he_uniform', input_shape=(cat_dim + cont_dim + noise_dim,)))
    H.add(BatchNormalization())
    H.add(Activation('relu'))

    H.add(Dense(256*16*16, kernel_initializer='he_uniform'))
    H.add(BatchNormalization())
    H.add(Activation('relu'))
    H.add(Reshape([16, 16, 256]))

    H.add(UpSampling2D(size=(2, 2)))
    H.add(Conv2D(256, (3, 3), padding='same', kernel_initializer='he_uniform'))
    H.add(BatchNormalization())
    H.add(Activation('relu'))

    H.add(UpSampling2D(size=(2, 2)))
    H.add(Conv2D(128, (3, 3), padding='same', kernel_initializer='he_uniform'))
    H.add(BatchNormalization())
    H.add(Activation('relu'))
...
def discriminator_model(cat_dim, cont_dim, dropout_rate = 0.25):
    ...
    H.add(Conv2D(128, (3, 3), strides=(2, 2), kernel_initializer='he_uniform', padding='same', activation='relu', input_shape=(64, 64, 1)))
    H.add(BatchNormalization())
    H.add(LeakyReLU(0.2))
    #H.add(Dropout(dropout_rate))

    H.add(Conv2D(256, (3, 3), strides=(2, 2), kernel_initializer='he_uniform', padding='same', activation='relu'))
    H.add(BatchNormalization())
    H.add(LeakyReLU(0.2))
    #H.add(Dropout(dropout_rate))

    H.add(Flatten())
    H.add(Dense(1024, kernel_initializer='he_uniform'))
    H.add(BatchNormalization())
    H.add(LeakyReLU(0.2))
    #H.add(Dropout(dropout_rate))

    disc_in = Input(shape=(64, 64, 1), name='in_image')
    features = H(disc_in)
